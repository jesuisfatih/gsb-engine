...
const navSections: NavSection[] = [
  ...
  },
  {
    title: "Commerce",
    items: [
      { icon: Package, label: "Products", name: "shopify-embedded-products" },
      { icon: Grid3x3, label: "Catalog & Surfaces", name: "shopify-embedded-catalog" },
  ...
  if (typeof window !== "undefined" && !window["app-bridge"]) {
    await Promise.race([
      ...
      new Promise<never>((_, reject) => 
        setTimeout(() => reject(new Error("app-bridge module load timeout")), 400)
      ),
    ]).catch(() => {
...
function waitForAppBridgeResources(timeout = 1000): Promise<ShopifyRuntimeResources> {
...
async function waitForLegacyReady(legacy: ShopifyGlobal, timeout = 500): Promise<void> {
...
async function retrieveSessionToken(options: {
  ...
  if (legacyApi?.idToken) {
    ...
    const token = await Promise.race([
      legacyApi.idToken(),
      new Promise<never>((_, reject) => setTimeout(() => reject(new Error("Shopify idToken timeout")), 1000)),
    ]);
    if (typeof token === "string" && token.length > 0) return token;
  }
